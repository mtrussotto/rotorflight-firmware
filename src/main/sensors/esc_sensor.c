...
#define ESC_SIG_CASTLE            0x0C // C is for castle.
...
static bool motorIsCastlePWM(void) {
...
{
    return escSensorPort != NULL || motorIsCastlePWM();
}
...
#define CASTLE_NTC_TEMP_SCALE 63.8125f // needs further processing.
#define CASTLE_NTC_R0 10000.0f
#define CASTLE_NTC_R2 10200.0f
#define CASTLE_NTC_B 3455.0f
...
static float castleDecodeTemperature(castleTelemetry_t* tele) {
    if (tele->linTempOrHalfMs < tele->ntcTempOrHalfMs) {
        ...
        // This is from the castle link documentation.  It's the "beta" method of calculating NTC
        // temperatures.
        // Reference temp = 298K (Has to be really 298.15K = 25C)
        // R0 = NTC resistance at reference temp = 10000
        // R2 = Reference resistance for voltage divider ADC input = 10200
...
static void castleDecodeTeleFrame(timeUs_t currentTimeUs, castleTelemetry_t* tele)
{
    ...
    DEBUG(ESC_SENSOR_DATA, DEBUG_DATA_VOLTAGE, tele->becVoltage);
    DEBUG(ESC_SENSOR_DATA, DEBUG_DATA_CURRENT, tele->becCurrent);
